...
if (await $`git status`.then(() => true, () => false)) {
    ...
    if (github.context.eventName === "push") {
        ...
        changedFiles = payload.commits.reduce<string[]>((a, x) => a.concat(x.added, x.modified, x.removed), [])
        ...
        changedFiles = data.flatMap(x => x.previous_filename ? [x.previous_filename, x.filename] : x.filename)
    } else {
...
const { stdout } = await $`cargo tree --workspace --depth 0`
...
const changedPackages = packages.filter(p => changedFiles.some(f => f.startsWith(p.path + sep)))
